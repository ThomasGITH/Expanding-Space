...
public class enemyBehaviour : MonoBehaviour {
    ...
    public GameObject bulletPrefab, powerUp, shield;
    public AudioClip shoot_sound, death_sound;
    public AudioSource track;
    GameObject player, home_base;
    Animator animator;
    ...
    void enemyMovement()
    {
        ...
        if (((player != null)&&(player.gameObject.GetComponent<movement>().health > 0))&& ((home_base.GetComponent<baseBehaviour>().health > 0)))
        {
                ...
                {
                   if ((player.gameObject.transform.position.x < gameObject.transform.position.x) && (playerHealth > 0))
                    {
                    moveX = -1;
                    animator.SetBool("walking", true); animator.SetBool("idle", false);
                    }
                    else if ((player.gameObject.transform.position.x > gameObject.transform.position.x) && (playerHealth > 0))
                    {
                    moveX = 1;
                    animator.SetBool("walking", true); animator.SetBool("idle", false);
                    }
